...
2. Все поля объявления соответствуют отправленным данным

### {#TC-POST-002}: Ошибка валидации (сохранение объявления с отрицательной ценой)  
- **Запрос:** `POST /api/1/item`   
...
### {#TC-POST-003}: Ошибка валидации (недопустимый sellerID в сохранении объявления)  
- **Запрос:** `POST /api/1/item`   
...
### {#TC-POST-004}: Ошибка валидации (пустое тело запроса)  
- **Запрос:** `POST /api/1/item`   
- **Тело (application/json) пустое:**  
```json
{}
```
- **Ожидаемый результат:**  
1. Status Code: `400 Bad Request`
2. Сообщение об ошибке созхранения объявления

### {#TC-POST-005}: Проверка тела ответа на сохранение объявления 
- **Запрос:** `POST /api/1/item`   
- **Тело (application/json) с валидными данными:**  
```json
{
